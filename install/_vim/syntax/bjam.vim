" Vim syntax file
" Language: bjam (specifically boost jam)
...
"
"   " bjam
"   au BufNewFile,BufRead *.jam,*.v2    setf bjam
...
sy keyword jamPseudo alias clean dirs exe file install lib
sy keyword jamPseudo obj shell uninstall using
sy keyword jamPseudo project requirements
...
sy cluster jamStringElement contains=jamStringSpecial
sy region jamString start=+"+ end=+"+ contains=jamStringSpecial,@spell


" Comments
" (only comments [by bjam standards] if they follow whitespace)
" sy match jamComment "\([ \t]\|^\)#.*" contains=jamTodo,jamVar,jamString
sy match jamComment "\([ \t]\|^\)#.*" contains=jamTodo
